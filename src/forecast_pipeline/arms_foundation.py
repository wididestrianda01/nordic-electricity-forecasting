...
class ChronosArm:
    ...
    def _covariates(features: pd.DataFrame) -> pd.DataFrame:
        ...
        if "regime" in frame.columns:
            frame = frame.copy()
            frame["regime"] = pd.factorize(frame["regime"], sort=True)[0].astype(float)
        return frame
    ...
    def predict_quantiles(
        ...
        if len(future_features) >= self.OUTPUT_CHUNK_LENGTH:
            future_covariates = frame_to_time_series(self._covariates(future_features))
...
class TimesfmArm:
    ...
    def predict_quantiles(
        ...
        """Forecast native P10/P50/P90 on the derived price-only grid."""
